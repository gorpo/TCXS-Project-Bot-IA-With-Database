...
import keyboard
from config import bot, version, bot_username, git_repo,logs,sudoers,bot_id
                ...
                if adm['user'] == True:
                    try:
                        ...
                        await bot.sendMessage(chat_id,
                                              f"🤖 `Ocorreu um erro ao inserir os dados na Database,este comando funciona somente para administradores, tente novamente e verifique se usou o comando da forma correta, qualquer espaço ou dados invalidos podem dar erro ao cadastrar, evitando assim dados invalidos na Database.Envie novamente o comando  exemplo:` ```restringir @doador id_doador dias``` ***Exemplo:*** restringir @xbadcoffee 1367451130 30 ",
                                              'markdown')
...
#SISTEMA PARA LIMPAR OS DOADORES CADASTRADOS NA DATABASE DE PERMANENCIA NO GRUPO DE DOADORES---------------------->
    ...
    try:## SISTEMA DE BANIMENTO--------------------------------------------------------------->
        cursor_sqlite.execute("""SELECT * FROM permanencia; """)
        resultados = cursor_sqlite.fetchall()
        for resutado in resultados:
            grupo_permanencia = resultados['id_grupo']
        chat_id = msg['chat']['id']
        chat_type = msg['chat']['type']
        id_usuario = msg['from']['id']
            ...
            if msg['text'] == 'verificar': #adm['user'] == False:
                ...
                for resutado in resultados:
                    ...
                    if hoje[0:2] == aviso[0:2]:
                        await bot.sendMessage(chat_id,f"🤖 {doador} ***Falta uma semana para você grupo, caso ainda tenha interesse em continuar usando a loja faça uma doação, envie o comprovante aqui no grupo que um de nossos administradores irá colocar mas dias em sua permanencia.***\n`Doador:` {doador}\n`Id_Doador:` {id_doador}\n`Início:` {data_inicial}\n`Termino:` {data_ban}\n`Data Aviso:` {aviso}\n`Permanência:` {dias}",'markdown')
                    ...
                    if hoje[3:5] == data_ban[3:5]:
                        ...
                        cursor_sqlite.execute(f"""DELETE FROM permanencia WHERE doador='{doador}'""")
                        conexao_sqlite.commit()
                        await bot.sendMessage(chat_id,f"🤖 ***Removido do grupo pois deu a sua permanência do grupo.***\n`Usuário:` {doador}\n`Id_Usuário:` {id_doador}\n`Início:` {data_inicial}\n`Termino:` {data_ban}\n`Permanência:` {dias}",'markdown')
